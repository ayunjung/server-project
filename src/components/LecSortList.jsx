import React from 'react'
import styled from 'styled-components'
import Pagebar from './Pagebar'
import { Link } from 'react-router-dom'

// 사진 주소 : "data:image/png;base64,"

const LecSmallMain = styled.div`
    display: flex;
    flex-wrap: wrap;
    width: 70%;
    height: 480px;
    margin: 20px auto 0;
`
const SBicLecture = styled.div`
    width : 220px;
    margin : 0 auto;
`

const SImgWrap = styled.div`
    width : 100%;
    height : 160px;
`

const SImg = styled.img`
    width : 100%;
    height : 160px;
`

const STitleP = styled.p`
    text-align : center;
    margin : 5px 0;
    font-weight : bold;
`

const SMakerP = styled.p`
    text-align : right;
    padding : 0 7px;
`

const Lec = ({ lec }) => {

    return (
        <SBicLecture>
            <Link to={`/LectureView/${lec.ulecnum}`} style={{ textDecoration: 'none', color: 'black', display:'block' }}>
                <SImgWrap>
                    <SImg src={lec.lecpic} alt="강의사진" />
                </SImgWrap>
                <STitleP>{lec.title}</STitleP>
                <SMakerP>{lec.teacher}</SMakerP>
            </Link>
        </SBicLecture>
    )
}


const LecSortList = ({filteredData, page, setPage, limit}) => {

    const offset = (page - 1) * limit;

    return (
        <>
            <LecSmallMain>
                {Array.from(filteredData.slice(offset, offset + limit)).map(lec => {
                    return (
                        <Lec lec={lec} key={lec.ulecnum}/>
                    )
                })}
            </LecSmallMain>
            <Pagebar total={filteredData.length} page={page} setPage={setPage} limit={4}/>
        </>
    )
}

export default LecSortList